...
/**
 * Transformer: Total Wireless Dynamic Media / Scene7 images.
 *
 * The source serves product/marketing imagery from Scene7 IS/Image
 * (e.g. https://s7d1.scene7.com/is/image/tracfone/...). Those URLs carry
 * rendition/template parameters that must survive the docx → markdown
 * round-trip, so we rewrite each DM <img> into an anchor. A companion
 * auto-block in scripts/scripts.js rebuilds responsive <picture> markup
 * from these anchors at render time.
 *
 * Runs in afterTransform ONLY — block parsers run between the hooks and
 * extract <img> references into block cells; rewriting imgs to anchors in
